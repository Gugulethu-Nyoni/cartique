...
export default class Cartique {
 constructor(products, features = {}, callbacks = {}) {
    ...
    // NEW: Variant state (must be initialized before use)
    this._variantSelections = {};  // productId -> variantId
    this._currentQuantity = {};    // productId -> quantity
...
/**
 * Get current variant (selected or default)
 */
getCurrentVariant(product) {
  // If no variants, return product as variant
  if (!product.variants || product.variants.length === 0) {
    return {
      id: product.id,
      price: product.price || 0,
  ...
  // Safety check: ensure _variantSelections exists
  if (!this._variantSelections) {
    this._variantSelections = {};
  }

  // Check if we have a stored selection for this product
  const selection = this._variantSelections[product.id];
  if (selection) {
    const variant = product.variants.find(v => v.id === selection);
    if (variant) return variant;
  }
...
/**
 * Get current price with bulk pricing applied
 */
getCurrentPrice(product, quantity = 1) {
  const variant = this.getCurrentVariant(product);
  let price = variant.price || product.price || 0;

  // Apply bulk pricing if set
  if (variant.bulkPrice && variant.bulkMinimumQty && quantity >= variant.bulkMinimumQty) {
    price = variant.bulkPrice;
  }
...
getCurrentInventory(product) {
  const variant = this.getCurrentVariant(product);
  return variant.inventory !== undefined ? variant.inventory : product.inventory || 0;
}



/**
 * Get available variant attributes for selector
 */
getVariantAttributes(product) {
  ...
  product.variants.forEach(variant => {
    if (variant.attributes && Array.isArray(variant.attributes)) {
      // Handle array format: [{ key: 'weight', value: '250g' }]
      variant.attributes.forEach(attr => {
        if (!attributes[attr.key]) {
          attributes[attr.key] = [];
        }
        if (!attributes[attr.key].includes(attr.value)) {
          attributes[attr.key].push(attr.value);
        }
      });
    } else if (variant.attributes && typeof variant.attributes === 'object') {
      // Handle object format: { weight: '250g' }
...
/**
 * Check if product has variant options to display
 */
hasVariantOptions(product) {
  if (!this.hasVariants(product)) return false;
  
  // Check if variants have different attributes
  const attrSets = product.variants.map(v => {
    if (v.attributes) {
      if (Array.isArray(v.attributes)) {
        return JSON.stringify(v.attributes.map(a => ({ key: a.key, value: a.value })).sort());
      } else if (typeof v.attributes === 'object') {
        return JSON.stringify(Object.keys(v.attributes).sort().map(k => ({ key: k, value: v.attributes[k] })));
      }
    }
    return '';
  });
...
/**
 * Render variant selector if product has variants
 */
renderVariantSelector(product) {
  ...
  if (currentVariant.attributes) {
    if (Array.isArray(currentVariant.attributes)) {
      currentVariant.attributes.forEach(attr => {
        currentAttrs[attr.key] = attr.value;
      });
    } else if (typeof currentVariant.attributes === 'object') {
      Object.assign(currentAttrs, currentVariant.attributes);
    }
  }

  let html = '<div class="cartique-variant-selector">';

  // Fix: Iterate over attribute keys properly
  Object.keys(attributes).forEach(attrKey => {
    const values = attributes[attrKey];
    // Capitalize the attribute key for display
    const label = attrKey.charAt(0).toUpperCase() + attrKey.slice(1);
    const currentValue = currentAttrs[attrKey] || values[0] || '';

    html += `
      <div class="cartique-variant-group" data-key="${attrKey}">
    ...
    values.forEach(value => {
      ...
      const variant = product.variants.find(v => {
        if (v.attributes) {
          if (Array.isArray(v.attributes)) {
            return v.attributes.some(a => a.key === attrKey && a.value === value);
          } else if (typeof v.attributes === 'object') {
            return v.attributes[attrKey] === value;
          }
        }
        return false;
      });
      ...
      // Fix: Display the actual value, not [object Object]
      html += `
        <div class="cartique-variant-option ${isSelected ? 'selected' : ''} ${isOutOfStock ? 'out-of-stock' : ''}"
             data-product-id="${product.id}"
             data-variant-id="${variant.id}"
             data-attr-key="${attrKey}"
             data-attr-value="${value}"
             onclick="cartiqueInstance.selectVariant(this)">
          ${value}
...
selectVariant(element) {
  ...
  // Update UI
  const container = element.closest('.cartique-variant-group');
  if (container) {
    container.querySelectorAll('.cartique-variant-option').forEach(el => {
...
/**
 * Render bulk pricing if set
 */
renderBulkPricing(product, quantity = 1) {
  const variant = this.getCurrentVariant(product);

  if (!variant.bulkPrice || !variant.bulkMinimumQty) return '';

  const isBulk = quantity >= variant.bulkMinimumQty;
  const displayPrice = isBulk ? variant.bulkPrice : variant.price;
  ...
  return `
    <div class="cartique-bulk-pricing">
      ...
      <div class="cartique-bulk-row">
        <span class="cartique-bulk-price">${this.currencySymbol}${variant.bulkPrice.toFixed(2)}</span>
        <span class="cartique-bulk-min">Min ${variant.bulkMinimumQty} items</span>
...
/**
 * Render quantity selector if enabled
 */
...
/**
 * Attach events to single product view
 */
attachSingleProductEvents(product) {
  ...
  // Quantity decrease
  container.querySelectorAll('.cartique-qty-decrease').forEach(btn => {
  ...
  // Quantity increase
  container.querySelectorAll('.cartique-qty-increase').forEach(btn => {
    btn.addEventListener('click', () => {
      const input = container.querySelector('.cartique-quantity-input');
      if (input) {
        let val = parseInt(input.value) || 1;
  ...
  const qtyInput = container.querySelector('.cartique-quantity-input');
  if (qtyInput) {
  ...
  // Add to cart
  const addBtn = container.querySelector('.spv-cartique_add_to_cart');
  if (addBtn) {
    addBtn.addEventListener('click', (e) => this.addToCart(e));
  }

  // Back button
  const backBtn = container.querySelector('.back-to-products');
  if (backBtn) {
    backBtn.addEventListener('click', () => this.returnToListView());
  }
  ...
  container.querySelectorAll('.tab-button').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const content = container.querySelector(`[data-tab-content="${tab}"]`);
...
/* END VIEW CART BLOCK */
  ...
  /**
 * Add to cart with variant and bulk support
 */
addToCart(event) {
  const button = event.target.closest('.cartique_add_to_cart, .spv-cartique_add_to_cart');
  if (!button) return;

  const productId = parseInt(button.dataset.productId) || parseInt(button.id);
  const product = this.products.find(p => p.id === productId);
  if (!product) return;

  // Get current values
  const variant = this.getCurrentVariant(product);
  const quantity = this._currentQuantity || 1;
  const price = this.getCurrentPrice(product, quantity);
  const image = this.getCurrentImage(product);
  const inventory = this.getCurrentInventory(product);
...
/**
 * Update single product view with new features
 */
      ...
      <div class="product-info-column">
        <div class="product-meta">
          ...
          <div class="price-container">
            <span class="price">${this.currencySymbol}${price.toFixed(2)}</span>
          </div>
        ...
        <button class="spv-cartique_add_to_cart" id="${variant.id || product.id}" data-product-id="${product.id}">
...
/**
 * Update display when variant or quantity changes
 */
updateSingleProductDisplay(product) {
  ...
  if (bulkContainer) {
    const newBulkHTML = this.renderBulkPricing(product, quantity);
    if (newBulkHTML) {
      bulkContainer.outerHTML = newBulkHTML;
    } else {
  ...
  if (btn) {
    btn.id = variant.id || product.id;
    btn.textContent = inventory > 0 ? 'ADD TO CART' : 'SOLD OUT';
    btn.disabled = inventory <= 0;
  }
}
